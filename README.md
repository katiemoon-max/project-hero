...
**Corollary — authored worked examples are NOT permitted (RULED by Katie, 18 Aug 2026, closing the Round 10 worked-example question).** Where a spec point has no past-paper worked example to carry (the board has barely examined it), the document ships without one: a missing example honestly reflects the examination record, and every example in the corpus stays traceable to a real paper and a real mark scheme. An example authored from the spec point and marking conventions would be SME-authored content entering a machine-consumed source-of-truth corpus, however clearly labelled — the class the fourth row above excludes. The F148/F150 remedy is unchanged: an unattributed example is rebuilt from a named scheme or deleted, never licensed.
