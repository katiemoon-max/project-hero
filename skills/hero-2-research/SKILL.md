...
1. **Corpus conversion complete AND table-integrity verified.** `corpus.conversion.status` must be `complete` and `corpus.known_casualties` current. Waves must never start against a partial corpus — R2's candidate selection and R3's sweep both assume every sitting is greppable. **Also confirm `corpus.conversion.table_gate_passed` is true**, and if the corpus predates that field (or was converted outside the pipeline), run `python scripts/convert_pdfs.py <corpus_root> --verify` here and read the verdict yourself. A mark scheme that converted to zero pipe characters has no table left: R2 will quote it confidently and be wrong about which marking point earns which mark (F17). Partial corpus fails loudly; a table-less one does not.
