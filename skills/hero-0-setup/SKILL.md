...
1. **Convert the §3 PDFs with `scripts/convert_pdfs.py`** (docling, table-structure recognition on, OCR off — resumable via a first-line marker, writes `<name>.md` next to each `<name>.pdf` with one `## Page N` section per page, flags scan-only docs that need OCR or a PDF-direct fallback). Usage: `python convert_pdfs.py <corpus_root>`; add `--subdir "Unit 1"` to shard, `--limit 3` to smoke-test first. It **overwrites `.md` left by any other converter**, so a corpus carrying an older text-layer conversion upgrades in place on re-run. Budget **~2–3 s per page on CPU** and a one-off ~500 MB model download — start it now, as background agents, so it runs while the vault builds. Convert ERs too where they exist; if the ER corpus is missing or unconverted, set `research_mode: nlm` (that is the only thing `nlm` covers).
...
5. Track `corpus.conversion.status` (`pending → running → complete`). **`complete` requires a passing table-integrity gate**, not just a finished run — record the gate verdict alongside the status. **`/hero-2-research` gates on `complete`** — waves must never start against a partial or table-less corpus.
...
- Directory skeleton: `research/`, `knowledge-files/`, `prompts/` (copy the seven stage prompts from `templates/prompts/`), `scripts/` (copy the seven `.py` scripts **and `requirements.txt`** — two scripts import `commonmark`, the converter needs `docling`)
