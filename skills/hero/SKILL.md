...
3. **Print a compact status block:**
   ...
   - **Derive, don't trust, every machine-checkable claim in `status` (F113):** re-audit state = count `re-audit.md` files against `knowledge-files/**/*.md` (one `find` each); untitled-callout or similar content blockers = run the grep, don't quote the entry. `project.json` is hand-maintained and the re-audit evidence lives on disk; nothing else reconciles them — a completed 14-file backfill stayed listed as outstanding for a day and 14 agent passes were nearly re-spent. Stale-pessimistic wastes tokens; stale-OPTIMISTIC ships unaudited content; the same gap produces both, so a checkable blocker is checked before it is printed, and every blocker entry carries the date it was written
