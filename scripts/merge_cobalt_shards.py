    ...
    for i, raw in enumerate(open(path, encoding="utf-8").read().splitlines(), 1):
        ...
        if line.startswith("#"):        # any other heading ends the current block
            ...
            if line.startswith("### "):  # shards must not emit the subtopic layer (F28)
                problems.append(f"{os.path.basename(path)}:{i} shard emits a '###' heading (schema forbids it): {line!r}")
            continue
        m = ENTRY.match(line)
        if m:
            if not cur_sp:
        ...
        if ENTRY_LOOSE.match(line):
            problems.append(f"{os.path.basename(path)}:{i} entry-like line did not parse (check the em-dash separators and ids): {line!r}")
            cur_entry = None
            continue
...
def main():
    ...
    if args.check:
        ...
        if newer:
            ...
            sys.exit(1)
        log(f"FRESH: merged file is newer than all {len(shards)} shard(s).")
        sys.exit(0)
    ...
    lines = [MARKER, "", f"# Cobalt commentary & Examiner Tips and Tricks -- merged by scripts/merge_cobalt_shards.py from {len(shards)} shard(s)", ""]
    emitted = 0
    for sbt_id, sbt in subtopics.items():
