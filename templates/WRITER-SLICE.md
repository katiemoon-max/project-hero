...
3. **`fixer_diff_sweep.py` on every fixer-touched master** — scans only the lines the fixer ADDED (git diff vs the pre-fixer state) for manufactured-certainty patterns and new quoted spans. Report-only: each hit is a read-before-strip candidate for the orchestrator. Why: a pilot-course fixer *introduced* a false "every time" claim after the checker signed off, and it shipped. Run while the pre-fixer version is still what git HEAD holds (or pass `--ref`)
4. **Sampled blind re-audit** — after the wave's fix cycle, 2–3 files (chosen for risk: dense sibling clusters, inferred figure content, heavy calculation) get a FRESH checker pass by an independent agent, blind to the original `checker-report.md`, writing to `re-audit.md`. Findings go through the normal ruling → fixer → diff-sweep path. Pilot-course calibration: ~10 actionable findings per re-audited file, yield independent of the original checker's model — the highest-yield quality step per token this pipeline has measured
