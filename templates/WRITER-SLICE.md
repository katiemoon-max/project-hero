...
4. **Sampled blind re-audit** — after the wave's fix cycle, 2–3 files (chosen for risk: dense sibling clusters, inferred figure content, heavy calculation) get a FRESH checker pass by an independent agent, blind to the original `checker-report.md` **and to contested rulings (F57 — a ruling in its brief makes the re-audit a second signature, not a second opinion; pass the ruling's evidence or withhold it)**, writing to `re-audit.md`. Findings go through the normal ruling → fixer → diff-sweep path. Calibration: pilot ~10 actionable findings per re-audited file; 1PH0 wave 1 13.3, every sampled file carrying a blocker its original checker missed — yield independent of the original checker's model, the highest-yield quality step per token this pipeline has measured
