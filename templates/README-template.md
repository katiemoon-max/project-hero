...
- Frontmatter: `section:` / `topic:` / `subtopic:` verbatim Cobalt names (precise scoping; avoids duplicate-SP-name traps across units), `tags: [<subject_slug>, project-hero]` — every value double-quoted, uniformly
- **SP names — THIS is the rule's single home (F35; `WRITER.md` defers here rather than restating it — two independent statements of one rule is how they drifted):** every heading that carries a spec-point name uses the bare `sp_name` field from `sp-mapping.json` — which holds what Cobalt holds. **Never the coordinate** (`sp_name` and `coord` are separate Cobalt fields, so `2.1.5.1 Acceleration` is a string that exists nowhere in Cobalt, and coordinates in H4s produce unpublishable student-facing prose), **never a filename fragment**. A name containing `&` keeps it verbatim at every heading level — the quoted name outranks the house "and-at-H3/H4" rule, precisely because it is quoted rather than chosen
- **Heading casing, one rule for every level:** fixed template words are Title Case at every heading level (`— In the Exam`, `Command Words for`, `Exam Strategy for`, `Multiple-Choice Questions on`); interpolated SP names appear verbatim. (H3 key-concept headings are also Title Case — a deliberate, recorded deviation from sentence-case house style)
- `# Sub-topic: <name>` / `## Spec Point: <name>`
- Spec text stays as the `> **Specification:** …` quote under each spec point heading. **The quote opens with the spec statement number as printed in the source (F38)**; a quote block may carry further statements on bare continuation lines, each opening with its own number — any tool parsing these blocks must read past the header line (F60)
...
> The exam-section H3 is the dash form `<Spec Point> — In the Exam` (F40): the retired `How <Spec Point> Appears in Exams` conjugated a singular verb onto whatever name was interpolated, and spec-point names are the board's — plural and gerund names produced headings that did not parse ("How Examples of Scalars & Vectors Appears in Exams"). The dash form is grammatical for any name and keeps the name available for retrieval.
...
- **ER citations as sitting + paper only** (e.g. "October 2022 Unit 1") — never file names. Question refs, scoped by where they appear (F46/F51): **running prose** = full month + main question number, no part letters ("June 2023 Q17"); **worked-example headings** = the full part reference ("2020 November Paper 1F, Q9(b)(ii)") — the heading is a student's lookup key; **levelled/extended-response refs** always carry their part letter + star ("2023 June Paper 1H Q9(c)*") — boards star a *part*, never a whole question, so a main-number-only star is inexpressible. Starred refs UNESCAPED ("Q*14" — see standing rule below)
